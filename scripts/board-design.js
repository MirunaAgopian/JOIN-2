...
function deactivateCheckboxesIfShowTaskIsOpen(index, cssClass) {
  if (isShowTaskActive) {
    ...
    document
      ...
      .classList.remove("subtask-checkbox");
  } else {
    if (cssClass == false) {
      document.getElementById(`subtask_${index}`).style = "display:block;";
      document
        .getElementById(`subtask_${index}`)
        .classList.add("subtask-checkbox");
    }
  }
}
    ...
    if (isClassActive == false) {
      for (let index = 0; index < amountOfSubtasks; index++) {
        if (document.getElementById(`subtask_${index}`).checked) {
          document
            .getElementById(`checkbox_${index}`)
            .classList.remove("checkbox-container");
          document
            .getElementById(`checkbox_${index}`)
            .classList.add("checkbox-checked");
        }
      }
    }
  }
...
function disableCssClass(id) {
  ...
  toggleCreatorLabelVisibility();
}

/**
 * Toggles the visibility of the creator label based on active Add‑Task CSS files.
 *
