...
async function saveDataInStore(dataObjStruct) {
  ...
  for (let index = 0; index < contactsKeysArr.length; index++) {
    if (
      dataObj.contactsResponse[contactsKeysArr[index]].mail ==
      dataObj.mailAddress
    ) {
...
function checkEditInputData(editDataObj) {
  for (let index = 0; index < editDataObj.keys.length; index++) {
    if (
      editDataObj.data.contactsResponse[editDataObj.keys[index]].mail ==
      editDataObj.data.mailAddress
    ) {
      let changedObj = getInputFieldsEditDialog();
      isMailValid = checkValidEmail(changedObj.mail);
      isTelValid = checkValidTel(changedObj.phone);
      if (
        editDataObj.data.contactsResponse[editDataObj.keys[index]].name !=
          changedObj.name ||
        editDataObj.data.contactsResponse[editDataObj.keys[index]].mail !=
          changedObj.mail ||
        editDataObj.data.contactsResponse[editDataObj.keys[index]].phone !=
          changedObj.phone
      ) {
        if (isMailValid && isTelValid) {
...
function showActivatedContactInList(contactArr, contactNumber) {
  ...
  if (contactNumber == 0) {
    for (let index = 0; index < contactArr.length; index++) {
      id = index + 1;
      document
        .getElementById("id_" + id)
        .classList.remove("contactID-activated");
      document
        .getElementById("spanId_" + id)
        .classList.remove("contact-name-override");
    }
  } else {
...
function changeColorOfActivatedContact(contactArr, contactNumber) {
  for (let index = 1; index <= contactArr.length; index++) {
    if (contactNumber == index) {
      document
        .getElementById("id_" + index)
        .classList.remove("contactID-hover");
      document
        .getElementById("id_" + index)
        .classList.add("contactID-activated");
      document
        .getElementById("spanId_" + index)
        .classList.add("contact-name-override");
    } else {
      document
        .getElementById("id_" + index)
        .classList.remove("contactID-activated");
